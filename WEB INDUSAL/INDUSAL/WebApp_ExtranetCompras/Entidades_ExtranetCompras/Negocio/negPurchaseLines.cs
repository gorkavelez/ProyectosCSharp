...
namespace INIKER.PurchaseLines
{
    ...
    public partial class PurchaseLines_Service : System.Web.Services.Protocols.SoapHttpClientProtocol
    {
        ...
        public PurchaseLines[] ReadMultiple(string Where, int pageSize, int numPage, string Sort)
        {
            bool flagResp = false;
            string lastReadKey = null;
            INIKER.PurchaseLines.PurchaseLines_Filter[] filtros = null;
            //Array.Resize(ref misFiltros, 0);
            PurchaseLines[] consultaPage; // = new AT_Solicitudes();
        ...
        private bool AplicarFiltros(string Where, ref INIKER.PurchaseLines.PurchaseLines_Filter[] misFiltros)
        {
            ...
            try
            {
                INIKER.PurchaseLines.PurchaseLines_Filter miFiltro;
                Array.Resize(ref misFiltros, 0);
                ...
                if ((Where != null) && Where.Length > 0)
                {
                    ...
                    {
                        miFiltro = new INIKER.PurchaseLines.PurchaseLines_Filter();
                        miFiltro.Field = INIKER.PurchaseLines.PurchaseLines_Fields.Document_No;
