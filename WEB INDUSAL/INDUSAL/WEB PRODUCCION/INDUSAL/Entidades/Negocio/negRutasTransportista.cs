...
namespace INIKER.RutasTransportista
{
    ...
    public partial class ListaRTransportistaINDUSAL_Service : System.Web.Services.Protocols.SoapHttpClientProtocol
    {
        ...
        //List<AT_Solicitudes> ??
        [System.ComponentModel.Description("Devuelve las Rutas por Transportista"), System.ComponentModel.DataObjectMethod(System.ComponentModel.DataObjectMethodType.Select, true)]
        public ListaRTransportistaINDUSAL[] ReadMultiple(string Where, int pageSize, int numPage, string Sort)
        {
            bool flagResp = false;
            string lastReadKey = null;
            INIKER.RutasTransportista.ListaRTransportistaINDUSAL_Filter[] filtros = null;            
            //Array.Resize(ref misFiltros, 0);
            ListaRTransportistaINDUSAL[] consultaPage; // = new AT_Solicitudes();
        ...
        private bool AplicarFiltros(string Where, ref INIKER.RutasTransportista.ListaRTransportistaINDUSAL_Filter[] misFiltros)
        {
            ...
            try
            {
                INIKER.RutasTransportista.ListaRTransportistaINDUSAL_Filter miFiltro;
                Array.Resize(ref misFiltros, 0);
                ...
                if ((Where != null) && Where.Length > 0)
                {
                    ...
                    {
                        miFiltro = new INIKER.RutasTransportista.ListaRTransportistaINDUSAL_Filter();
                        miFiltro.Field = INIKER.RutasTransportista.ListaRTransportistaINDUSAL_Fields.Tipo_relacion;
                        miFiltro.Criteria = strFiltros[0];
                        Array.Resize(ref misFiltros, misFiltros.Length + 1);
                        misFiltros[misFiltros.Length - 1] = miFiltro;
                    }
                    //1: Cod_relacion (código transportista - código cliente)
                    if (strFiltros.Length > 1 && (strFiltros[1] != null) && strFiltros[1] != string.Empty)
                    {
                        miFiltro = new INIKER.RutasTransportista.ListaRTransportistaINDUSAL_Filter();
                        miFiltro.Field = INIKER.RutasTransportista.ListaRTransportistaINDUSAL_Fields.Cod_relacion;
                    ...
                    {
                        miFiltro = new INIKER.RutasTransportista.ListaRTransportistaINDUSAL_Filter();
                        miFiltro.Field = INIKER.RutasTransportista.ListaRTransportistaINDUSAL_Fields.Cod_ruta;
