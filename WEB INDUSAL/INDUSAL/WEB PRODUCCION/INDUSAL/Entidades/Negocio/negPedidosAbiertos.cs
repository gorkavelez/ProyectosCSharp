...
namespace INIKER.PedidosAbiertos
{
    ...
    public partial class ListaPedidosAbiertosINDUSAL_Service : System.Web.Services.Protocols.SoapHttpClientProtocol
    {
        ...
        public ListaPedidosAbiertosINDUSAL[] ReadMultiple(string Where, int pageSize, int numPage, string Sort)
        {
            bool flagResp = false;
            string lastReadKey = null;
            INIKER.PedidosAbiertos.ListaPedidosAbiertosINDUSAL_Filter[] filtros = null;            
            //Array.Resize(ref misFiltros, 0);
            ListaPedidosAbiertosINDUSAL[] consultaPage; // = new AT_Solicitudes();
        ...
        private bool AplicarFiltros(string Where, ref INIKER.PedidosAbiertos.ListaPedidosAbiertosINDUSAL_Filter[] misFiltros)
        {
            ...
            try
            {
                INIKER.PedidosAbiertos.ListaPedidosAbiertosINDUSAL_Filter miFiltro;
                Array.Resize(ref misFiltros, 0);
                ...
                if ((Where != null) && Where.Length > 0)
                {
                    ...
                    //0: Sell_to_Customer_No
                    if ((strFiltros[0] != null) && strFiltros[0] != string.Empty)
                    {
                        miFiltro = new INIKER.PedidosAbiertos.ListaPedidosAbiertosINDUSAL_Filter();
                        miFiltro.Field = INIKER.PedidosAbiertos.ListaPedidosAbiertosINDUSAL_Fields.Sell_to_Customer_No;
                    ...
                    {
                        miFiltro = new INIKER.PedidosAbiertos.ListaPedidosAbiertosINDUSAL_Filter();
                        miFiltro.Field = INIKER.PedidosAbiertos.ListaPedidosAbiertosINDUSAL_Fields.No;
                    ...
                    {
                        miFiltro = new INIKER.PedidosAbiertos.ListaPedidosAbiertosINDUSAL_Filter();
                        miFiltro.Field = INIKER.PedidosAbiertos.ListaPedidosAbiertosINDUSAL_Fields.Posting_Date;
                        //miFiltro.Criteria = ParCaseSensitive + strFiltros[2];
                        Array.Resize(ref misFiltros, misFiltros.Length + 1);
                        misFiltros[misFiltros.Length - 1] = miFiltro;
                    }
                    //3: Pedido_en_preparacion
                    if (strFiltros.Length > 3 && (strFiltros[3] != null) && strFiltros[3] != string.Empty)
                    {
                        miFiltro = new INIKER.PedidosAbiertos.ListaPedidosAbiertosINDUSAL_Filter();
                        miFiltro.Field = INIKER.PedidosAbiertos.ListaPedidosAbiertosINDUSAL_Fields.Pedido_en_preparacion;
                        miFiltro.Criteria = strFiltros[3];
                        Array.Resize(ref misFiltros, misFiltros.Length + 1);
                        misFiltros[misFiltros.Length - 1] = miFiltro;
                    }
