...
{    
    public partial class Maquina3 : Form
    {
        ...
        private void Maquina3_Load(object sender, EventArgs e)
        {
            ...
            txtDia.Text = DateTime.Today.ToString("dd/MM/yy").Trim();
            TxtHora.Text = DateTime.Now.ToString("HH:00").Trim();
        ...
        public bool guardarDatosHistorico()
        {
            ...
            try
            {
                ...
                //messageBox("valores guardados, Sabana: "+ sabana)                                             
                cFunWeb.GuardarDatosenNav(valores,
                                          sabana,                                  //sabana
                                          usuario,                                 //usuario
                                          DateTime.Parse(txtDia.Text.Trim()),      //fecha registro
                                          DateTime.Parse(TxtHora.Text.Trim()),     //hora registro
        ...
        public void CargarTodoFormularioDatosNav()
        {
            try
            {
                ...
                string filtrohora = TxtHora.Text.Trim();
                //MessageBox.Show("filtro fecha datos en hora: " + filtrofecha+", filtro hora: "+filtrohora);
                funcionesProxy.traerRegistrosUltimaHora(_sabana, filtrofecha, filtrohora, ref datoshistorico);                                                
                //MessageBox.Show(datoshistorico.Count().ToString());
                if (datoshistorico.Count() == 0)
                {                    
                    //si no hay registros con la misma hora a la que nos movemos sacará los datos de uno en uno.
                    datoshistorico = null;
                    filtrohora = hora.AddHours(-1).ToString("HH:00") + "|" + TxtHora.Text; //sacamos la hora anterior                              
                    funcionesProxy.traerRegistrosUltimaHora(sabana, filtrofecha, filtrohora, ref datoshistorico);
                    if (datoshistorico.Count() != 0)
                ...
                {   
                    ...
                    funcionesProxy.TraerObservaciones(sabana + ";" + filtrofecha + ";" + filtrohora, ref arrObservaciones);
        ...
        private void txt_sk_194_Enter(object sender, EventArgs e)
        {
            if (txt_sk_194.Text == string.Empty)
                txt_sk_194.Text = hdn_194.Text;            
        }

        private void txt_sk_195_Enter(object sender, EventArgs e)
        {
            if (txt_sk_195.Text == string.Empty)
                txt_sk_195.Text = hdn_195.Text;
        }
